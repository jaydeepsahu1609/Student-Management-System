...
if ($pwd != $repwd) {
    ...
    if ($conn) {
        ...
        if (!$num) {
            $iq = "INSERT INTO request(username, fullname, password, user_type) VALUES('$usr', '$name', '$pwd', '$user_type');";
            $res = mysqli_query($conn, $iq);

            if ($res) {
                echo "
                    <script>window.alert('Request has been sent to the admin for verification. It usually takes about 30minutes for confirmation. Try logging in after 30minutes.');
